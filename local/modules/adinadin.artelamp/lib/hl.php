...
Class HLAdditional
{
	const NOT_HL_ID = 'Не установлен ID HL блока.';
	const EMPTY_DATA = "Не заполнен массив";
	const EMPTY_ELEMENT_ID = "Не передан ID элемента";
	...
	public function getList ($params = array())
	{
		...
		if ($this->haveValue($params['HL_ID'], 'not') || intval($params['HL_ID']) <= 0) {
			if ($this->haveValue($this->HL_ID, 'not'))
				$this->obErrors->setError(self::NOT_HL_ID);
		} else {
		...
		if ($this->obCache->InitCache($cacheTime, $cacheId, $cachePath)) {
			...
			if ($arUses['select']) {
				...
				$this->main_query->setSelect(array('*'));
			}
			...
			if ($arUses['navigation']) {
				if ($this->haveValue($params['navigation']['nPageTop']) && intval($params['navigation']['nPageTop']) > 0) {
					$this->main_query->setLimit($params['navigation']['nPageTop']);
				} elseif ($this->haveValue($params['navigation']['nPageSize']) && $this->haveValue($params['navigation']['iNumPage'])) {
					$this->main_query->setLimit($params['navigation']['nPageSize']);
					$this->main_query->setOffset(($params['navigation']['iNumPage'] - 1) * $params['navigation']['nPageSize']);
				}
			}
	...
	public function add ($params = array())
	{
		...
		if ($this->haveValue($params['arData'], 'not'))
			$this->obErrors->setError(self::EMPTY_DATA);
	...
	public function update ($params = array())
	{
		...
		if ($this->haveValue($params['ID'], 'not'))
			$this->obErrors->setError(self::EMPTY_ELEMENT_ID);
	 ...
	 */
	public function getHLList ($select = array('ID', 'NAME'))
	{
		$HLList = false;
		...
		$res = HighloadBlockTable::getList(array('select' => $select));
	 ...
	 * @return array
	 */
	public function getHLFields ($params = array())
	{
		$this->obErrors->clearErrors();
		...
		if ($this->obCache->InitCache($cacheTime, $cacheId, $cachePath)) {
			...
			$res = \CUserTypeEntity::GetList(array(), array('ENTITY_ID' => 'HLBLOCK_' . $this->HL_ID, 'LANG' => 'ru'));

			while ($arField = $res->Fetch()) {
				$arFields[] = array('CODE' => $arField['FIELD_NAME'], 'NAME' => $arField['LIST_COLUMN_LABEL'], 'FULL_EL' => $arField);
