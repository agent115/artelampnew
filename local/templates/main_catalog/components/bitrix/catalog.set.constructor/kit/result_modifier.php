...
//добаляем элементы из OTHERS в default  добавляем DEFAULT_SET_IDS
if(isset($arResult["SET_ITEMS"]['OTHER'] )){
	foreach ($arResult["SET_ITEMS"]['OTHER'] as $key=>$arItem){
		$arResult["DEFAULT_SET_IDS"][]=$arItem['ID'];
		$arResult["SET_ITEMS"]['DEFAULT'][]=$arItem;
	}
}

//пересчитываем цену на набор
$totalPrice=0;
foreach ($arResult["SET_ITEMS"]['DEFAULT'] as $key=>$arItem){
	$totalPrice+=$arItem["PRICE_DISCOUNT_VALUE"];
}
$totalPrice+=$arResult['ELEMENT']["PRICE_DISCOUNT_VALUE"];
$arResult['TOTAL_PRICE']=CurrencyFormat($totalPrice, "RUB");
