...
///*
        $res = CIBlockElement::GetList(array(), array("ID" => $ElementID), false, false, array("PROPERTY_SERIYA"));
        if ($ar = $res->GetNext()) {
            global $arrFilterBest;
            $arrFilterBest["ID"] = array();
            if ($ar["PROPERTY_SERIYA_VALUE"] != "") {
                $res = CIBlockElement::GetList(array("RAND" => "ASC"), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE" => "Y", "!ID" => $ElementID, "PROPERTY_SERIYA" => $ar["PROPERTY_SERIYA_VALUE"]), false, array("nPageSize" => 4), array("ID", "NAME"));
                while ($arItem = $res->GetNext()) {
                    $arrFilterBest["ID"][] = $arItem["ID"];
                }
            }
        }

                if (count($arrFilterBest["ID"]) > 0) {
                    $APPLICATION->IncludeComponent(
                        "bitrix:catalog.section",
                        "element_detail_recommendation",
                        Array(
          ...
          /*  } else {
                ...
                if (!empty($arRecomData)) {
                    if (ModuleManager::isModuleInstalled("sale") && (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N')) {
                        ?><? $APPLICATION->IncludeComponent("bitrix:catalog.bigdata.products", "detail_recommendation", array(
