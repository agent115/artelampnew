...
//получаем количество по каждому элементу
foreach ($arResult['ITEMS'] as $key=>$arItem){
	$itemsIds[]=$arItem['ID'];
}

$arOrder = array("SORT" => "ASC", 'NAME' => 'ASC');
$arFilter = array('ACTIVE' => 'Y', "IBLOCK_ID"=>19, 'PROPERTY_DOP_OFFER'=>$itemsIds);
...
foreach ($arResult['ITEMS'] as $key=>$arItem){
	foreach($arElements as $arElement){
		if($arItem['ID'] == $arElement['PROPERTY_DOP_OFFER_VALUE']){
			$arResult['ITEMS'][$key]['NEW_QUANTITY']=$arElement['PROPERTY_DOP_OFFER_COUNT_VALUE'];
		}
	}
}